...
export async function getMealPlans() {
  try {
    ...
    console.log('Meal plans fetched successfully:', data?.length || 0);
...
export async function getMealPlansByCategory(category) {
  try {
    console.log('Fetching meal plans by category:', category);
    const { data, error } = await supabase
      .from('meal_plans')
      .select('*')
    ...
    console.log(`Meal plans for category ${category} fetched:`, data?.length || 0);
    return data
  } catch (error) {
    console.error(`Error fetching meal plans for category ${category}:`, error)
    return null
